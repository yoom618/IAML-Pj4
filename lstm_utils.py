...
def rnn_cell(rnn_cell_size, dropout_keep_prob, residual, is_training):
...
def _get_final_index(sequence_length, time_major=True):
  indices = [tf.maximum(0, sequence_length - 1),
             tf.range(tf.shape(sequence_length)[0])]
  if not time_major:
    indices = indices[-1::-1]
...
def set_final(sequence, sequence_length, values, time_major=False):
  ...
  mask = tf.sequence_mask(
      ...
      maxlen=tf.shape(sequence_batch_major)[1],
      dtype=tf.float32)
  sequence_batch_major = (
...
def initial_cell_state_from_embedding(cell, z, name=None):
  ...
  return nest.pack_sequence_as(
      cell.zero_state(batch_size=tf.shape(z)[0], dtype=tf.float32),
      tf.split(
          tf.layers.dense(
              z,
...
def get_sampling_probability(hparams):
...
def maybe_split_sequence_lengths(sequence_length, num_splits, total_length):
  ...
  if sequence_length.shape.ndims == 1:
    ...
    with tf.control_dependencies([tf.Assert(tf.reduce_all(tf.logical_or(tf.equal(sequence_length, 0), tf.equal(sequence_length, total_length))),
            data=[sequence_length])]):
      sequence_length = (tf.tile(tf.expand_dims(sequence_length, axis=1), [1, num_splits]) // num_splits)
    ...
    sequence_length.set_shape([tf.shape(sequence_length)[0], num_splits])
  else:
    raise ValueError(
